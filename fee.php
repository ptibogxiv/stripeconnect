...
llxHeader('', $langs->trans("StripeFeeList"));


$form = new Form($db);
$salstatic = new PaymentStripeFee($db);
$societestatic = new societe($db);
$acc = new Account($db);

$sql = "SELECT ";
//$sql "u.rowid as uid, u.lastname, u.firstname, u.login, u.email, u.admin, u.salary as current_salary, u.fk_soc as fk_soc,";
$sql.= " s.rowid, s.fk_soc, s.stripe_fee, s.application_fee, s.mode, s.datep as datep, s.datev as datev, s.fk_typepayment as type, s.num_payment, s.fk_bank,";
$sql.= " ba.rowid as bid, ba.ref as bref, ba.number as bnumber, ba.account_number, ba.fk_accountancy_journal, ba.label as blabel,";
$sql.= " pst.code as payment_code";
$sql.= " FROM ".MAIN_DB_PREFIX."stripeconnect_transaction as s";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_paiement as pst ON s.fk_typepayment = pst.id";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."bank as b ON s.fk_bank = b.rowid";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."bank_account as ba ON b.fk_account = ba.rowid";
//$sql.= " ,".MAIN_DB_PREFIX."user as u";
$sql.= " WHERE ";
//$sql " u.rowid = s.fk_user AND ";
$sql.= " s.entity = ".$conf->entity." AND s.mode=".$mode." ";
...
if ($result)
{
	...
	print_barre_liste($langs->trans("StripeFeeList"),$page,$_SERVER["PHP_SELF"],$param,$sortfield,$sortorder,'',$num, $totalnboflines, 'title_accountancy.png', 0, '', '', $limit);
	...
	print '</td>';
  	// Label
	print '<td class="liste_titre"><input type="text" class="flat" size="27" name="search_label" value="'.$db->escape($search_payment).'"></td>';
	// Date
    ...
    print_liste_field_titre("Customer",$_SERVER["PHP_SELF"],"u.rowid","",$param,"",$sortfield,$sortorder);
    print_liste_field_titre("Payment",$_SERVER["PHP_SELF"],"s.num_payment","",$param,'align="left"',$sortfield,$sortorder);
    ...
    // Label payment
        print "<td>".dol_trunc($obj->num_payment,40)."</td>\n";
		// Date payment
  ...
  print $acc->getNomUrl(1);
        ...
        // Amount
        print "<td align=\"right\">".price($obj->stripe_fee)."</td>";
        print "<td align=\"right\">".price($obj->application_fee)."</td>";
        print "</tr>\n";

        $total = $total + $obj->stripe_fee;
        $total2 = $total2 + $obj->application_fee;
    ...
    print '<td class="liste_total" align="right">'.price($total).'</td><td class="liste_total" align="right">'.price($total2).'</td>';
	print "<td></td></tr>";
