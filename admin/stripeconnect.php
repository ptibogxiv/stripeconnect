...
if ($action == 'setvalue' && $user->admin)
{
  ...
  $result=dolibarr_set_const($db, "STRIPE_TEST_WEBHOOK_CONNECT_ID",GETPOST('STRIPE_TEST_WEBHOOK_CONNECT_ID','alpha'),'chaine',0,'',0);
	...
	if (! $result > 0) $error++;                                                
  $result=dolibarr_set_const($db, "STRIPE_LIVE_WEBHOOK_ID",GETPOST('STRIPE_LIVE_WEBHOOK_ID','alpha'),'chaine',0,'',0);
	if (! $result > 0) $error++;
	$result=dolibarr_set_const($db, "STRIPE_LIVE_WEBHOOK_KEY",GETPOST('STRIPE_LIVE_WEBHOOK_KEY','alpha'),'chaine',0,'',0);
	if (! $result > 0) $error++;
  $result=dolibarr_set_const($db, "STRIPE_LIVE_WEBHOOK_CONNECT_ID",GETPOST('STRIPE_LIVE_WEBHOOK_CONNECT_ID','alpha'),'chaine',0,'',0);
	if (! $result > 0) $error++;
...
$stripearrayofwebhookevents=array('account.updated','payout.created','payout.paid','charge.pending','charge.refunded','charge.succeeded','charge.failed','payment_intent.succeeded','payment_intent.payment_failed','source.chargeable','customer.deleted');
...
$endpoint->enabled_events = $stripearrayofwebhookevents;
if ( GETPOST('webhook','alpha') == $conf->global->STRIPE_TEST_WEBHOOK_CONNECT_ID ) {
if ( empty( GETPOST('status','alpha') )) {
$endpoint->disabled = true; 
} else {
$endpoint->disabled = false;
}}
$endpoint->url = dol_buildpath('/public/stripe/ipn.php?connect&test', 3);
$endpoint->save();
if ($endpoint->status == 'enabled')
{
	print '<a href="'.$_SERVER['PHP_SELF'].'?action=ipn&webhook='.$endpoint->id.'&status=0">';
...
$endpoint->enabled_events = $stripearrayofwebhookevents;
if ( GETPOST('webhook','alpha') == $conf->global->STRIPE_LIVE_WEBHOOK_CONNECT_ID ) {
if ( empty( GETPOST('status','alpha') )) {
$endpoint->disabled = true; 
} else {
$endpoint->disabled = false;
}}
$endpoint->url = dol_buildpath('/public/stripe/ipn.php?connect', 3);
$endpoint->save();
if ($endpoint->status == 'enabled')
{
	print '<a href="'.$_SERVER['PHP_SELF'].'?action=ipn&webhook='.$endpoint->id.'&status=0">';
